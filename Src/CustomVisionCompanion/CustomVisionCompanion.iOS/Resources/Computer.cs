...
namespace CustomVisionCompanion.iOS {
	...
	/// </summary>
	public class ComputerInput : NSObject, IMLFeatureProvider
	{
		...
		public ComputerInput (CVPixelBuffer data)
		{
			if (data == null)
	...
	/// </summary>
	public class ComputerOutput : NSObject, IMLFeatureProvider
	{
		static readonly NSSet<NSString> featureNames = new NSSet<NSString> (
		...
		public ComputerOutput (NSDictionary<NSObject, NSNumber> loss, string classLabel)
		{
			if (loss == null)
	...
	public class Computer : NSObject
	{
		...
		public Computer ()
		{
			var url = NSBundle.MainBundle.GetUrlForResource ("Computer", "mlmodelc");
			NSError err;

			model = MLModel.Create (url, out err);
		}

		Computer (MLModel model)
		{
			this.model = model;
		}

		public static Computer Create (NSUrl url, out NSError error)
		{
			if (url == null)
		...
		/// </summary>
		/// <param name="input">an instance of ComputerInput to predict from</param>
		/// <param name="error">If an error occurs, upon return contains an NSError object that describes the problem.</param>
		public ComputerOutput GetPrediction (ComputerInput input, out NSError error)
		{
			...
			return new ComputerOutput (lossValue, classLabelValue);
		}
		...
		/// <param name="error">If an error occurs, upon return contains an NSError object that describes the problem.</param>
		public ComputerOutput GetPrediction (CVPixelBuffer data, out NSError error)
		{
			var input = new ComputerInput (data);
